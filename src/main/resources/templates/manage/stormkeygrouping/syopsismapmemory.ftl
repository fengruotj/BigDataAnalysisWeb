...
<html>
<head>
    <title>SyopsisHashMap内存使用统计图表</title>
    <meta charset="utf-8">
...
<div class="container">
    <div class="jumbotron">
        <h2>SyopsisHashMap内存使用UI显示</h2>
        <div class="row center-block">
            <div class="btn-group" role="group" aria-label="...">
                <button id="showresult" type="button" class="btn btn-primary">显示结果</button>
                <button id="savereslut" type="button" class="btn btn-primary" data-toggle="modal"
                        data-target="#saveresultModal">保存当前结果
                </button>
            </div>
        </div>
    </div>
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        echartsRedBrokenLineInit(myChart, {}, 'SyopsisHashMap内存使用测试结果(单位 字节)');
...
<script type="text/javascript">
    ...
    function showEcharts() {
        var myChart = echarts.init(document.getElementById('main'));
        var url = "/echarts/SyopsisMapMemory"
        $.ajax({
            url: url,
            dataType: 'json',
            data: {},
            ...
            success: function (data) {
                echartsRedBrokenLineInit(myChart, data, 'SyopsisHashMap内存使用测试结果(单位 字节)');
                $('#loaddata').modal('hide')
            },
